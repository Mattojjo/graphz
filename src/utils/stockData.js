...
export const generatePriceMovement = (currentPrice, volatility = 0.02) => {
    ...
    return Math.max(currentPrice + currentPrice * (drift + shock), MIN_PRICE);
};
...
export const generateHistoricalDataForTimeframe = (basePrice, timeframe, points = 150) => {
    const tfSec  = Math.floor(getTimeframeMs(timeframe) / 1000);
...
export const generateHistoricalData = (basePrice, points = 100) =>
    generateHistoricalDataForTimeframe(basePrice, '1m', points);
...
export const updateStockTick = (stock, timeframe = '1m', simNowSec = null) => {
    ...
    return {
        ...
        changePercent: ((newPrice - stock.basePrice) / stock.basePrice) * 100,
        historicalData,
    };
};
